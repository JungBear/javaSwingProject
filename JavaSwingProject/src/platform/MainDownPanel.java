...
public class MainDownPanel extends JPanel{
	...
	public MainDownPanel(String name, int x, int y, MainFrame mainF) {
		...
		case "cloth":
			...
			// 카테고리별 페이지 만들기
			scpp1 = new SmallCategoryPreviewPanel(productService.readData(name, smallCategoryBtn1.getText()));
			scpp2 = new SmallCategoryPreviewPanel(productService.readData(name, smallCategoryBtn2.getText()));
			scpp3 = new SmallCategoryPreviewPanel(productService.readData(name, smallCategoryBtn3.getText()));
			scpp4 = new SmallCategoryPreviewPanel(productService.readData(name, smallCategoryBtn4.getText()));
			break;
		case "cosmetic":
			//스킨케어, 클렌징, 네일, 맨즈케어
